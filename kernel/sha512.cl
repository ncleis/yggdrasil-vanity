...
// Функция для 64-битного циклического сдвига вправо
inline ulong rotate_right64(ulong x, uint n) {
    return (x >> n) | (x << (64 - n));
}

// bitselect: для каждого бита, если c==1, то b, иначе a
#define choose(x, y, z) (bitselect(z, y, x))
// Определяет "majority" vote, с учётом, что x == z
#define bit_maj(x, y, z) (bitselect(x, y, ((x) ^ (z))))

// Макрос для байтового обмена (реверс байтов в ulong)
#define SWAP(x) as_ulong(as_uchar8(x).s76543210)

// Определяем rotate через нашу функцию для 64-битных чисел
#define rotr64(x, n) rotate_right64(x, (uint)(n))
...
// Adapted for ulongs
// Паддинг для сообщения в 128-битном виде
...
/* Стандартный паддинг, INPLACE.
   Добавляет 1-бит, затем исходную длину сообщения в конце блока.
...
static int md_pad_128(ulong *msg, const long msgLen_bytes) {
  ...
  ulong length_high = 0; // Верхние 64 бита (при msgLen_bytes < 2^61)
